...
/* qa / ask-codex (애니메이션) */
@keyframes pulseGlow { 0%,100% { transform:scale(1); opacity:.9; } 50% { transform:scale(1.05); opacity:1; } }
@keyframes blink { 0%,100% { opacity:1; } 50% { opacity:0; } }
@keyframes floatUp { 0% { opacity:0; transform:translateY(16pt); } 100% { opacity:1; transform:translateY(0); } }
.ask-kick { font-size:15pt; color:#9aa0c0; letter-spacing:3pt; text-align:center; }
.ask-big { font-size:56pt; font-weight:800; text-align:center; animation:pulseGlow 2.4s ease-in-out infinite; }
.ask-pill { display:inline-block; margin-top:28pt; font-size:18pt; color:#bfeaff; background:rgba(0,180,255,.1); border:1.5pt solid rgba(0,180,255,.42); border-radius:30pt; padding:13pt 30pt; animation:floatUp .9s ease-out both; }
.ask-cursor { animation:blink 1s step-end infinite; font-weight:400; }
/* cmd (복사 가능한 명령어) */
.li.cmdline { padding-left:0; margin-bottom:8pt; }
.li.cmdline::before { display:none; }
.cmd { display:flex; align-items:center; gap:9pt; background:rgba(0,180,255,.10); border:1px solid rgba(0,180,255,.34); border-radius:9pt; padding:7pt 8pt 7pt 12pt; }
...
/* phones (폰 세로 캡처 필름스트립) */
.phrow { display:flex; justify-content:center; align-items:flex-start; gap:24pt; margin-top:16pt; }
.phone { display:flex; flex-direction:column; align-items:center; }
.phone img { height:236pt; width:auto; border-radius:13pt; border:1px solid rgba(255,255,255,.16); box-shadow:0 6pt 22pt rgba(0,0,0,.5); }
.pcap { margin-top:10pt; font-size:13pt; color:#cdd1ea; text-align:center; line-height:1.32; max-width:200pt; }
.pcap b { color:#fff; }
"""

COPY_JS = ('<script>function cp(b){var t=b.previousElementSibling.textContent;'
           'function ok(){var o=b.innerHTML;b.classList.add("ok");b.innerHTML="\\u2713 \\ubcf5\\uc0ac\\ub428";'
           ...
           'document.body.appendChild(a);a.focus();a.select();try{document.execCommand("copy");ok();}catch(e){}document.body.removeChild(a);}</script>')

HEAD = ('<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8">'
        '<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/static/pretendard.min.css">'
        '<style>%s</style>%s</head>') % (CSS, COPY_JS)
...
def phones(kick, title, items):
    # 폰 세로 캡처 필름스트립: items = [(img, caption), ...] 를 같은 높이로 나란히
    cells = "".join('<div class="phone"><img src="img/%s" alt=""><div class="pcap">%s</div></div>'
                    % (img, cap) for img, cap in items)
    inner = ('<div class="frame"><div class="kicker">%s</div><div class="h1">%s</div>'
             '<div class="rule"></div><div class="phrow">%s</div></div>'
             ) % (esc(kick), esc(title), cells)
    page(inner)
...
def qa(big, sub):
    inner = ('<div class="qmark">?</div><div class="frame" style="justify-content:center;">'
             '<div class="cl-title grad">%s</div><div class="cl-sub">%s</div></div>') % (esc(big), esc(sub))
    page(inner)

def ask_codex(kick, big, pill):
    inner = ('<div class="frame" style="justify-content:center;">'
             '<div class="ask-kick">%s</div>'
             '<div class="ask-big grad" style="margin-top:16pt;">%s</div>'
             '<div style="text-align:center;"><span class="ask-pill">%s<span class="ask-cursor">▋</span></span></div></div>'
             ) % (esc(kick), esc(big), esc(pill))
    page(inner)
...
chat(
    kick="판단 ② 멈춤 버튼 · 이렇게 멈춥니다",
    title="AI가 스스로 멈추는 순간",
    turns=[
        ("me", "이 상품들 정리해서 쿠팡에 바로 등록까지 해줘.", ""),
        ("ai", "초안은 다 만들었어요. 그런데 '<b>등록</b>' 버튼은 제가 안 누를게요 — 게시·결제·발송은 사장님 확인이 원칙이에요.", "여기서 멈춤"),
        ("me", "왜? 그냥 눌러도 되는데.", ""),
        ("ai", "되돌리기 어려운 일이라서요. 마지막은 사람이 보는 게 안전해요. 확인하시면 눌러주세요.", "되돌릴 수 없는 일"),
    ],
    punch="믿고 맡기되, 되돌릴 수 없는 버튼은 사람이.",
)
...
# (A안) 부품 6장(Cron·Hook·Subagent·MCP·Tool·Skill/Workflow glossary2)은 부록 용어 termpage(6~12)와
# 중복 + PART 1 "외우지 마" 표지와 모순이라 제거. 위 임팩트 statement 한 장이 그 요약·다리 역할을 한다.
# PART 2 — 이렇게 한다 (실제 스킬로 일 시키기)
section(2, "PART 2 · 이렇게 한다", "직원을 만들어\n일을 시킨다", "앞의 4빈칸(방향·업무·순서·지식)을 채우고 · 외부연결·브라우저·비서까지 — 틀은 [언제]·[나라면]·[예시]", CYAN)
...
chat(
    kick="PART 2 · D 실전",
    title="한 번 연결하면, 복붙이 사라집니다",
    turns=[
        ("me", "재고를 구글시트에 적어두는데, AI한테 물어보면 그걸 몰라서 매번 복사해서 붙여줘야 해.", ""),
        ("ai", "구글시트를 <b>연결</b>하면 복붙 없이 제가 바로 읽어요. 지금 연결해드릴까요?", "외부 서비스 연결"),
        ("me", "응, 연결해줘.", ""),
        ("ai", "로그인 창은 사장님이 직접 눌러 허용만 해주세요(비번은 제가 안 만져요). 이제 '재고 10개 미만만 골라줘'도 바로 됩니다.", "로그인은 사람 · 바로 읽음"),
    ],
    punch="한 번 연결하면, 복붙은 끝입니다.",
)
...
chat(
    kick="PART 2 · E 실전",
    title="한 번 보여주면, 나머진 AI가 클릭합니다",
    turns=[
        ("me", "관리자에서 매일 주문 100개를 하나하나 클릭해서 상태를 바꾸는데 미치겠어.", ""),
        ("ai", "지금 <b>로그인된 크롬 그대로</b> 쓸 거라 비밀번호는 안 만져요. 딱 한 번만 순서를 보여주실래요?", "학습모드 · 비번 안 만짐"),
        ("me", "이 주문 클릭 → '발송완료' 누르고 저장. 이렇게.", ""),
        ("ai", "익혔어요. 나머지 99개도 같은 순서로 대신 돌릴게요. 이상한 항목은 멈추고 물어볼게요.", "익힌 순서 재현"),
    ],
    punch="한 번 보여주면, 나머진 AI가 대신 클릭합니다.",
)
...
# PART 3 — 준비·쓰기
section(3, "PART 3 · 준비하고, 쓰기", "맨 땅에서 시작해,\n매일 쓰기까지", "처음 한 번만 깔면, 그다음은 폴더 열고 말만 걸면 됩니다", VIOLET)
bullets("처음 한 번 · 큰 그림", "딱 세 단계 — 그다음은 매일 똑같아요", [
    ("<b>① 앱 깔고 로그인</b> — Codex 데스크톱 앱. 딱 한 번. (다음 장 <b>실제 명령</b>)", ""),
    ("<b>② 작업폴더 만들고 · 복붙 3줄</b> — 코치와 내 폴더를 한 번에 세팅. (그다음 장)", "c"),
...
imgslide("① 처음 켜면 이 화면", "하는 일을 골라도 되고, 그냥 '건너뛰기' 눌러도 돼요",
    "app-onboarding.png", "안 골라도 아무 문제 없어요. 넘어가면 바로 시작합니다.")
imgslide("① 이런 앱이 열려요", "왼쪽에 메뉴, 가운데 대화창 — 여기서 다 합니다",
    "app-project.png", "왼쪽 '프로젝트'에 내 작업폴더를 넣고, 아래 칸에 말만 걸면 됩니다.")
imgslide("② 작업폴더를 '프로젝트'로 추가", "왼쪽 '프로젝트'에서 폴더를 고르고 이름 붙이기",
    "app-project-name.png", "폴더 하나(예: 내사업)를 고르고 이름을 지은 뒤 초록 테두리의 '저장'.")
imgslide("② 터미널 열기 — 오른쪽 위 사이드바 버튼", "따로 까만 창 열 필요 없어요",
    "app-plusmenu.png", "오른쪽 위 사이드바 버튼을 누르면 '터미널'이 있어요(초록 테두리). 앱 안에서 열립니다.")
bullets("② 그 터미널에 이 세 줄 붙여넣기", "오른쪽 '복사' 눌러 붙여넣고 Enter → 끝", [
...
bullets("처음 켤 때 · 권한을 '나 대신 승인'으로", "자기 PC에서 혼자 쓰면 — 막힘없이 빠르게", [
    ...
    ("<b>⚠️ 안전은 사람이</b> — 결제·발송·삭제·게시 버튼은 늘 사장님. 코치도 위험한 건 멈추고 물어봐요.", "v"),
])
imgslide("권한 · '나 대신 승인' 고르기", "입력칸 아래 드롭다운 → '나 대신 승인'(초록 테두리)",
    "app-permission.png", "'승인 요청'은 매번 물어봐 답답해요. 자기 PC면 '나 대신 승인'.")
imgslide("③ 코치 부르기 — 말로, 또는 '$'", "그냥 '~하고 싶어' 한마디면 코치가 알아서",
    "app-callskill.png", "콕 집으려면 입력칸에 '$' → 코치 목록. 막막하면 'ai-agent-maker'.")
imgslide("그림도 만들어요", "'이런 이미지 만들어줘' 하면 대화창에 바로",
    "app-image.png", "로그인만 있으면 돼요(API키 필요 없음). 상세컷·썸네일에 그대로.")
...
imgslide("정해진 시각에 자동 · '예약됨'", "왼쪽 '예약됨' → 오른쪽 위 '채팅으로 만들기'",
    "app-schedule.png", "일일 브리핑·주간 검토 같은 템플릿도 있어요.")
imgslide("예약은 대화로 만들어요", "'매일 아침 9시에 ~'처럼 말하면 그 시각에 알아서",
    "app-schedule2.png", "무인 실행이라 읽기·정리 같은 안전한 일만. PC·앱 켜진 동안 돌아요.")
statement("밖에서도, 폰으로\n내 PC 직원에게 일 시키기", "설정에서 한 번 켜고 QR만 스캔 — GitHub 필요 없어요", grad=True)
imgslide("① 왼쪽 아래 계정 → '설정'", "폰 연결은 여기서 시작해요",
    "app-settings-menu.png", "왼쪽 맨 아래 내 계정을 누르면 '설정'(초록 테두리)이 있어요.")
imgslide("② 설정 → '연결' → 켜고 '추가'", "'이 PC 제어' 탭에서 '원격 제어 연결 허용'을 켜요",
    "app-connections.png", "허용을 켜고(초록) '추가'(초록)를 누르면 QR이 나와요.")
imgslide("③ PC 화면에 이 QR이 떠요", "이제 폰을 꺼내서 이걸 스캔할 거예요",
    "app-qr.png", "'전화' 탭의 QR — 폰으로 찍으면 연결됩니다. (아래는 폰 화면 순서)")
phones("④ 폰에서 (1/2) · Remote 켜기", "폰 ChatGPT 앱에서 이 순서로 누르세요", [
    ("mobile-remote-menu.png", "<b>① 'Remote'</b> 탭"),
    ("mobile-remote-setup.png", "<b>② '데스크톱에서<br>로그인했습니다'</b>"),
    ("mobile-pairing.png", "<b>③ '페어링 코드가<br>있습니다'</b>"),
])
phones("④ 폰에서 (2/2) · 스캔하면 끝", "PC 화면의 QR을 폰으로 찍으면 연결돼요", [
    ("mobile-scan.png", "<b>④ PC의 QR을 스캔</b>"),
    ("mobile-connected.png", "<b>⑤ 연결!</b> 내 PC가 폰에 —<br>이제 밖에서도 '채팅'으로 시켜요"),
])
...
# 부록 — 용어 사전 (해부도 + 용어 12장)
section("✦", "부록 · 참고", "용어 사전", "이 개념들만 알아두면 AI한테 일 시키기가 한결 편해져요", CYAN)
